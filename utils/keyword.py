...
import math
from .text import canonical_tokens_from_text, contains_cjk, extract_cjk_ngrams
from ..core.config import env

def extract_keywords(text: str, min_length: int = 3) -> Set[str]:
    tokens = canonical_tokens_from_text(text)
    keywords = set()
    effective_min = 2 if contains_cjk(text) else min_length

    for token in tokens:
        if len(token) >= effective_min:
            keywords.add(token)
            if len(token) >= 3:
                for i in range(len(token) - 2):
                    keywords.add(token[i : i+3])
            if contains_cjk(token) and len(token) >= 4:
                for i in range(len(token) - 3):
                    keywords.add(token[i : i + 4])
    for i in range(len(tokens) - 1):
        bigram = f"{tokens[i]}_{tokens[i+1]}"
        if len(bigram) >= effective_min:
            keywords.add(bigram)
